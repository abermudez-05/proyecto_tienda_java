...
public class ServicioCarritoImpl implements ServicioCarrito{
	...
	public void agregarProducto(int idUsuario, long idProducto, int cantidad) {
		...
		List<Carrito> productosCarrito = usuario.getProductos_carrito();
		...
		for(Carrito pc : productosCarrito) {
			if(pc.getPelicula().getId() == idProducto) {
		...
		if(!productoEnCarrito) {
			...
			productoCarrito.setPelicula(entityManager.find(Pelicula.class, idProducto));
	...
	@Override
	public void quitarProductoCarrito(int idUsuario, Long id) {
		entityManager.createNativeQuery(ConstantesSQL.SQL_ELIMINAR_PRODUCTO_CARRITO).setParameter("usuario_id", idUsuario).setParameter("pelicula_id", id).executeUpdate();
	}
