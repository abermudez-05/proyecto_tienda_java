...
public class ServicioPedidosImpl implements ServicioPedidos{
	...
	public void confirmarPedido(int idUsuario) {
		...
		List<Carrito> c = entityManager.createQuery("select c from Carrito c where c.usuario.id = :usuario_id").setParameter("usuario_id", u.getId()).getResultList();
		...
		for(Carrito productoCarrito : c) {
			...
			pp.setPelicula(productoCarrito.getPelicula());
			pp.setPedido(p);
	...
	@Override
	public void procesarPaso1(String nombre, String direccion, String provincia, String postal, String telefono, String observaciones, int idUsuario) {
		Pedido p = obtenerPedidoIncompletoActual(idUsuario);
	...
	@Override
	public ResumenPedido procesarPaso3(String generoFavorito, String peliculaFavorita, int idUsuario) {
		Pedido p = obtenerPedidoIncompletoActual(idUsuario);
		p.setGeneroFavorito(generoFavorito);
		p.setPeliculaFavorita(peliculaFavorita);
		entityManager.merge(p);
	...
	private ResumenPedido obtenerResumenDelPedido(int idUsuario) {
		...
		resumen.setPostal(p.getPostal());
		resumen.setTelefono(p.getTelefono());
		resumen.setGeneroFavorito(p.getGeneroFavorito());
		resumen.setPeliculaFavorita(p.getPeliculaFavorita());
		...
		resumen.setPeliculas(servicioCarrito.obtenerProductosCarrito(idUsuario));
		return resumen;
	}
